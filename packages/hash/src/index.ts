...
export async function sha512(data: string | Uint8Array): Promise<string> {
  const bytes = typeof data === "string" ? new TextEncoder().encode(data) : data;
  if (typeof globalThis.crypto?.subtle !== "undefined") {
    const hashBuf = await globalThis.crypto.subtle.digest("SHA-512", bytes as unknown as BufferSource);
    return bufferToHex(new Uint8Array(hashBuf));
  }
  try {
    const nodeCrypto = await import("node:crypto");
...
export async function sha1(data: string | Uint8Array): Promise<string> {
  const bytes = typeof data === "string" ? new TextEncoder().encode(data) : data;
  if (typeof globalThis.crypto?.subtle !== "undefined") {
    const hashBuf = await globalThis.crypto.subtle.digest("SHA-1", bytes as unknown as BufferSource);
    return bufferToHex(new Uint8Array(hashBuf));
  }
  try {
    const nodeCrypto = await import("node:crypto");
...
export async function hmacSha256(key: string | Uint8Array, message: string | Uint8Array): Promise<string> {
  ...
  if (typeof globalThis.crypto?.subtle !== "undefined") {
    ...
    const signature = await globalThis.crypto.subtle.sign("HMAC", cryptoKey, msgBytes as unknown as BufferSource);
...
// Synchronous, zero-dependency, works everywhere
// ==========================================
...
export function md5(data: string): string {
  // Convert UTF-8 string to binary string representation
