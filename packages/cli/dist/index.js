...
function removeBackground(options) {
  ...
  const outPath = options.outputPath || path2.join(
    path2.dirname(inputPath),
    `${path2.basename(inputPath, path2.extname(inputPath))}-transparent.png`
  );
  const rawBuffer = fs3.readFileSync(inputPath);
  if (rawBuffer.readUInt32BE(0) !== 2303741511) {
    const transparentPng = createTransparentSamplePng(300, 300);
    fs3.writeFileSync(outPath, transparentPng);
    return {
      outputPath: path2.resolve(outPath),
  ...
  try {
    ...
    if (idatBuffers.length > 0 && ihdrWidth > 0 && ihdrHeight > 0) {
      ...
      fs3.writeFileSync(outPath, outBuffer);
      return {
        outputPath: path2.resolve(outPath),
        originalSize: stat.size,
  ...
  fs3.writeFileSync(outPath, fallback);
  return {
    outputPath: path2.resolve(outPath),
    originalSize: stat.size,
