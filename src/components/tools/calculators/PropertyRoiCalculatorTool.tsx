...
export default function PropertyRoiCalculatorTool() {
  ...
  const results = useMemo<PropertyRoiResults>(() => {
    try {
    ...
    <Card className="border-border/60 shadow-sm bg-card/40 backdrop-blur">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <Input id="price" type="number" value={price} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPrice(e.target.value)} />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="rent">Monthly Rental Income ($)</Label>
          <Input id="rent" type="number" value={rent} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setRent(e.target.value)} />
        </div>
        <div className="space-y-1.5">
