...
export default function FreelanceRateCalculatorTool() {
  ...
  const results = useMemo<FreelanceRateResult>(() => {
    try {
    ...
    <Card className="border-border/60 shadow-sm bg-card/40 backdrop-blur">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <Input id="goal" type="number" value={goal} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setGoal(e.target.value)} />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="expenses">Annual Business Expenses & Taxes ($)</Label>
          <Input id="expenses" type="number" value={expenses} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setExpenses(e.target.value)} />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="vacation">Vacation Weeks Per Year</Label>
          <Input id="vacation" type="number" value={vacation} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setVacation(e.target.value)} />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="billable">Billable Hours Per Week</Label>
          <Input id="billable" type="number" value={billable} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setBillable(e.target.value)} />
        </div>
        </div>
