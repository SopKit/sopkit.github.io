...
import { AlertCircle, BookOpen, CheckCircle, Download, Globe, HelpCircle, Link, Loader2, Music, Play, Shield, Star, Zap } from "lucide-react";
...
export default function DouyinDownloader() {
	...
	const handleDownload = async () => {
		if (!url.trim()) { setError("Please enter a Douyin video URL"); return; }
		if (!url.includes("douyin.com")) { setError("Please enter a valid Douyin URL"); return; }
		...
		try {
			...
			setVideoData({
				title: data.title || "Douyin Video", thumbnail: data.thumbnail,
		...
		} catch (err) { setError("Failed to process the Douyin video. Please try again."); }
	...
	return (
		<div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
			<Card><CardHeader><CardTitle className="flex items-center gap-2"><Link className="h-5 w-5" />Douyin Video Downloader</CardTitle></CardHeader>
				<CardContent className="space-y-4">
					<div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
						<Input type="url" placeholder="Paste Douyin video URL here..." value={url} onChange={(e) => setUrl(e.target.value)} className="flex-1" />
					...
					{videoData && <div className="space-y-4">
						<div className="flex items-center gap-2 p-3 bg-muted/50 border"><CheckCircle className="h-4 w-4" /><span className="text-sm">Douyin video processed successfully!</span></div>
					...
					<div className="text-xs text-muted-foreground"><p>Download Douyin videos in HD quality</p><p>No watermark downloads available</p><p>Free and no registration required</p></div>
				</CardContent>
			</Card>
			<Card><CardHeader><CardTitle className="flex items-center gap-2"><BookOpen className="h-5 w-5" />How to Download Douyin Videos</CardTitle></CardHeader>
				<CardContent><div className="space-y-4">
					{[{step:1,title:"Find the Douyin Video",desc:"Open Douyin and find the video you want."},{step:2,title:"Copy the URL",desc:"Copy the video URL from the share menu."},{step:3,title:"Paste and Download",desc:"Paste the URL above and click download."},{step:4,title:"Save Your Video",desc:"The video will download to your device."}].map((item) => (
			...
			<Card><CardHeader><CardTitle className="flex items-center gap-2"><Zap className="h-5 w-5" />Features</CardTitle></CardHeader>
				<CardContent><div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
					<div className="flex items-start gap-3"><div className="w-8 h-8 bg-gray-100 shrink-0 items-center justify-center"><Play className="h-4 w-4" /></div><div className="min-w-0"><h4 className="font-semibold">No Watermark</h4><p className="text-sm text-muted-foreground">Download without the Douyin watermark</p></div></div>
			...
			<Card><CardHeader><CardTitle className="flex items-center gap-2"><HelpCircle className="h-5 w-5" />FAQs</CardTitle></CardHeader>
				<CardContent className="space-y-4">
					...
					<div className="space-y-3"><h4 className="font-semibold">Can I remove the watermark?</h4><p className="text-sm text-muted-foreground">Yes, our downloader provides watermark-free downloads.</p></div>
			...
			<Card><CardHeader><CardTitle className="flex items-center gap-2"><Star className="h-5 w-5" />User Reviews</CardTitle></CardHeader>
				<CardContent><div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
					{[{name:"Li W.", review:"Best Douyin downloader! Works perfectly.", date:"Dec 2024",r:5},{name:"Chen Y.", review:"No watermark downloads - exactly what I needed.", date:"Nov 2024",r:5},{name:"Maria K.", review:"Fast and reliable. Highly recommended!", date:"Nov 2024",r:4},{name:"Alex P.", review:"Simple interface, great results.", date:"Oct 2024",r:5}].map((r,i) => (
