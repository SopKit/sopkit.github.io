...
export default function BlutvDownloader() {
	...
	const handleDownload = async () => {
		if (!url.trim()) { setError("Please enter a bluTV URL"); return; }
		...
		try {
			...
			setVideoData({
				title: data.title || "bluTV Video", thumbnail: data.thumbnail,
		...
		} catch (err) { setError("Failed to process the bluTV video. Please try again."); }
	...
	return (
		<div className="w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
			<Card><CardHeader><CardTitle className="flex items-center gap-2"><Link className="h-5 w-5" />bluTV Video Downloader</CardTitle></CardHeader>
				<CardContent className="space-y-4">
					<div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
						<Input type="url" placeholder="Paste bluTV video URL here..." value={url} onChange={(e) => setUrl(e.target.value)} className="flex-1" />
					...
					{videoData && <div className="space-y-4">
						<div className="flex items-center gap-2 p-3 bg-muted/50 border"><CheckCircle className="h-4 w-4" /><span className="text-sm">bluTV video processed successfully!</span></div>
					...
					<div className="text-xs text-muted-foreground"><p>Download bluTV videos in HD quality</p><p>Fast processing and download speeds</p><p>Free and no registration required</p></div>
				</CardContent>
			</Card>
			<Card><CardHeader><CardTitle className="flex items-center gap-2"><BookOpen className="h-5 w-5" />How to Download bluTV Videos</CardTitle><CardDescription>Step-by-step guide</CardDescription></CardHeader>
				<CardContent><div className="space-y-4">
					{[{step:1,title:"Find the bluTV Video",desc:"Open bluTV and find the video you want to download."},{step:2,title:"Copy the Video URL",desc:"Copy the URL from your browser or the share button."},{step:3,title:"Paste and Download",desc:"Paste the URL above and click download."},{step:4,title:"Save Your Video",desc:"Choose your preferred quality and save the video."}].map((item) => (
			...
			<Card><CardHeader><CardTitle className="flex items-center gap-2"><HelpCircle className="h-5 w-5" />FAQs</CardTitle><CardDescription>Common questions</CardDescription></CardHeader>
				<CardContent className="space-y-4">
					<div className="space-y-3"><h4 className="font-semibold">Is this bluTV downloader free?</h4><p className="text-sm text-muted-foreground">Yes, 100% free with no registration or hidden fees.</p></div>
