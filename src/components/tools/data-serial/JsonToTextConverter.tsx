...
export default function JsonToTextConverter() {
  ...
  return (
    <Card className="border-border/60 shadow-sm">
      <CardHeader>
        <CardTitle className="text-lg">JSON to Text Converter</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <Textarea className="min-h-[220px] font-mono text-sm" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Paste JSON here..." />
        <Button type="button" onClick={run}>Convert to formatted text</Button>
        <Textarea className="min-h-[220px] font-mono text-sm bg-muted/30" readOnly value={output} placeholder="formatted text output" />
      </CardContent>
    </Card>
  );
}
