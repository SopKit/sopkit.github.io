...
export interface ToolWithScore extends Tool {
	searchScore?: number;
...
export async function GET(request: Request) {
	...
	try {
		...
		const searchResults = allTools
			.map((tool: any) => {
				...
				if (!query) {
					...
					tool.longTailKeywords?.forEach((longTail: string) => {
		...
		console.error("Search API error:", _error);
		return Response.json(
...
function generateSearchSEOTitle(query: string, category: string, resultCount: number): string {
	if (!query) {
		return category === "all"
...
function generateSearchSEODescription(query: string, category: string, resultCount: number): string {
	if (!query) {
		return category === "all"
