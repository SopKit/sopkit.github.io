 ...
 */
function generateSummary(transcript: string, title: string, duration: string) {
...
function extractKeyPoints(transcript: string) {
	...
	const keyPoints = sentences
		.filter((s: string) => keywords.some((kw: string) => s.toLowerCase().includes(kw)))
		.slice(0, 5)
		.map((s: string) => s.trim());
	...
	if (keyPoints.length < 3) {
		...
		return sentences
			.filter((_: string, i: number) => i % step === 0)
			.slice(0, 5)
			.map((s: string) => s.trim());
	}
 ...
 */
function generateInsights(transcript: string, title: string) {
