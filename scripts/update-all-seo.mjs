...
const updateAllTools = () => {
		...
		Object.keys(toolsData.categories).forEach((categoryKey) => {
			...
			if (category.tools && Array.isArray(category.tools)) {
				category.tools.forEach((tool) => {
					...
					// Build a highly-optimized long-tail, privacy-centric SEO Description
	...
	console.log(`Successfully updated SEO metadata & keywords for all ${updatedCount} tools across all categories in tools.json!`);
};
